package io.reactorbrains.demo.sec10;
...
public class Lec04RetryWhenAdvanced {
                        ...
                        flux -> flux
                                ...
                                .handle((rs, synchronousSink) -> {
                                    if (rs.failure().getMessage().equals("500")) {
                                        synchronousSink.next(1); // en cas d'erreur 500 on autorise le retry
                                    } else {
                                        synchronousSink.error(rs.failure()); // en cas d'erreur 404 on n'autorise pas de retry
                                    }
                                })
    ...
    private static Mono<String> orderService(String ccNumber) {
        return Mono.fromSupplier(() -> {
            processPayment(ccNumber); // propage une exception si erreur de paiement
